...
def train(net, x, y, num_iter=5000, lr=0.009):
    ...
    for i in range(num_iter):
        ...
        optimizer.step()
        if i % 500 == 0:
            print(f"[ReLU] Итерация {i}, Ошибка: {loss.item():.4f}")
    return loss.item()
...
# === Параметры и запуск ===
net = MLPptorchReLU(3, 50, 20, 10, 3)
train(net, X_tensor, Y_tensor)
print(f"[ReLU] Ошибки на обучении: {evaluate(net, X_tensor, Y_tensor)}")
print(f"[ReLU] Ошибки на тесте: {evaluate(net, X_test_tensor, Y_test_tensor)}")